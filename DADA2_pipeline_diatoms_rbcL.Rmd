...
subtitle: "DAY 1: Exploring a DADA2 bioinformatic pipeline for diatoms metabarcoding"
output: html_notebook
---


**Prerequisites and help for using Rnotebook/Rmarkdown**  

This is an [R Markdown](http://rmarkdown.rstudio.com) Notebook. When you execute code within the notebook, the results appear beneath the code. Execute chunks by clicking the *Run* button within the chunk (or the more classical approach: line by line by placing your cursor on it and pressing *Ctrl+Enter*). To insert new chunk.s, click the *Insert Chunk* button on the toolbar or press *Ctrl+Alt+I*. When you save the notebook, an HTML file containing the code and output will be saved alongside it (click the *Preview* button or press *Ctrl+Shift+K* to preview the HTML file). The preview shows you a rendered HTML copy of the contents of the editor. Consequently, unlike *Knit*, *Preview* does not run any R code chunks. Instead, the output of the chunk when it was last run in the editor is displayed.



**Regarding the pipeline**

The bioinformatic pipeline below is the one developped by Callahan and adapted by François KECK for diatoms metabarcoding.  
You can find the original tutorial of the pipeline by B. CALLAHAN on its GitHub page: <https://benjjneb.github.io/dada2/tutorial.html>  
You can find the diatoms metabarcoding adaptation by F. KECK on its GitHub page: <https://github.com/fkeck/DADA2_diatoms_pipeline/blob/master/pipeline.R>
...
# build path to the data once the primers were detected and removed:
path_cut <- "<path to raw data which have been processed by cutadapt>"
if(!dir.exists(path_cut)) dir.create(path_cut)

# build path to the data after filtration of the reads
...
*N.B.: In this case, multiple primers couples are used to amplify rbcL from diatoms and cover the whole of the diversity (Vasselon et al., 2017)*  
...
# rbcL Forward primers:
Fw1 <- "AGGTGAAGTAAAAGGTTCWTACTTAAA" # Diat_rbcL_708F_1
Fw2 <- "AGGTGAAGTTAAAGGTTCWTAYTTAAA" # Diat_rbcL_708F_2
Fw3 <- "AGGTGAAACTAAAGGTTCWTACTTAAA" # Diat_rbcL_708F_3
...
# rbcL Reverse primers:
Rv1 <- "CCTTCTAATTTACCWACWACTG" # R3_1
Rv2 <- "CCTTCTAATTTACCWACAACAG" # R3_2
...
#### FOR WINDOWS YOU MAY USE THIS COMMAND ####
for (i in seq_along(raw_F_reads)){
cutadapt <- "py -m cutadapt"
system(paste0(cutadapt, " ",
              R1_flags, " ",
              R2_flags, " ",
              "--discard-untrimmed", " ",
              "--max-n 0", " ",
...
# alternatives on Linux: using pattern detection and count lines:
# grep -c "^+$" path_to_raw/*.(common_files_extension)
# grep -c "^+$" path_to_cut/*.(common_files_extension)
...
# Here, according to the expected amplicon length (263 pb) we need to adapt truncLen for then be able 
...
#### GOOD TO KNOW (especially for big data) ####
# If you want to save the list to continue the pipeline another day
# 
### saving the list as RDS-like file, run:
# saveRDS(merged_reads, "~/DADA2_training_J1/merged_reads_chunk_9")
# 
### load the RDS file as list, run:
# merged_reads <- readRDS("~/DADA2_training_J1/merged_reads_chunk_9")
# 
...
Here a little sum up: we used a MiSeq run (Illumina) giving 2x250pb as outputs (R1 & R2).  
We removed the 27 or 22 first nucleotides by removing the primers (see CHUNK 4), the remaining reads have thus a 223 and 228pb-length, respectively.  
From that, we truncated at 200pb (R1) and 170pb (R2).  
The barcodes we target (without primers) have a 263pb mean length (can slightly vary through species).  
It means that the mean overlapping length should be around 107pb : [ (R1 length + R2 length) - barcode length ]  
*N.A.: (200+170)-263 = 107*
```{r}
# Let's check that!
...
# m value from 12(+1) (default overlapping length for DADA2 is 12) until 170(+1) (maximum length of reads we have, could logically not exceed that)
for (m in 13:171){
  j<-j+1
  vec_numb <- vector()
  vec_abun <- vector()
...
max(maxov)
## mean overlapping length -> should be close to the expected value: 107pb
mean(meanov)
```
...
```{r}
# based on previous chunk, let's keep ASV with an overlapping length between 100 and 110pb
for (i in 1:length(merged_reads)){
  merged_reads[[i]] <- merged_reads[[i]] %>% filter(nmatch>100 & nmatch<110)
}
...
```{r}
#### installing reference databases for diatoms: diatbarcode from F. KECK Github page ####
...
tax <- assignTaxonomy(seqtab.nochim, tax_fas$path, minBoot = 75,
...
finalComplete_tab1[,10:(ncol(finalComplete_tab1)-1)] <- sapply(finalComplete_tab1[,10:(ncol(finalComplete_tab1)-1)], as.numeric)
...
finalComplete_tab2[,11:ncol(finalComplete_tab2)] <- sapply(finalComplete_tab2[,11:ncol(finalComplete_tab2)], as.numeric)
...
# melt function: really useful for drawing barplots (using reshape2 package)
library(reshape2)
melted_SynthTab <- melt(SynthTab)

# equivalent of melt function (using dplyr & tidyr packages)
SynthTab_plot <- SynthTab %>% gather(key = variable,
                               value = value,
                               colnames(SynthTab)[3:ncol(SynthTab)])
...
finalComplete_tab2[,11:ncol(finalComplete_tab2)] <- sapply(finalComplete_tab2[,11:ncol(finalComplete_tab2)],
                                                          as.numeric)

#### In all samples, remove ASV which presents an abundance below 50 ####
finalComplete_tab_b50 <- finalComplete_tab2
for (i in colnames(finalComplete_tab_b50)[11:ncol(finalComplete_tab_b50)]){
  finalComplete_tab_b50[[i]][which(finalComplete_tab_b50[[i]]<50,arr.ind=T)] <- 0
}
finalComplete_tab_b50 <- finalComplete_tab_b50[which(rowSums(finalComplete_tab_b50[,11:ncol(finalComplete_tab_b50)])>0),]
...
finalComplete_tab_b50 <-  finalComplete_tab_b50 %>%
  gather(key = variable, value = value, colnames(finalComplete_tab_b50)[10:ncol(finalComplete_tab_b50)]) %>%
  group_by(Empire, Kingdom, Subkingdom, Phylum, Class, Order, Family, Genus, Species, variable) %>%
